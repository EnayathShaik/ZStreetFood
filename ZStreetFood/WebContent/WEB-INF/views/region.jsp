...
<table>
	...
	<tr>
		<td colspan="2">
			<c:if test="${!empty region.name}">
				<input type="submit"
					value="<spring:message text="Edit Region"/>" />
			</c:if>
			<c:if test="${empty region.name}">
				<input type="submit"
					value="<spring:message text="Add Region"/>" />
			</c:if>
		</td>
	</tr>
</table>	
...
<br>
<h3>Regions List</h3>
<c:if test="${!empty listRegions}">
	<table class="tg">
	<tr>
		<th width="80">Region ID</th>
	...
	<c:forEach items="${listRegions}" var="region">
		...
		<tr>
			...
			<td>${region.name}</td>
			<td>${region.code}</td>
			<td><a href="<c:url value='/region/edit/${region.id}' />" >Edit</a></td>
			<td><a href="<c:url value='/region/remove/${region.id}' />" >Delete</a></td>
		</tr>
	</c:forEach>
	</table>
</c:if>
